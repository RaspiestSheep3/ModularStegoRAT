...
def HandleClient(clientSocket : socket.socket):
    ...
    if(requestType == "DEFINE_NEW_USER"):
        ...
        while(shopping):
            ...
            elif(clientRequest["Type"] == "BROWSE_SHOP"):
                lowerModuleID = clientRequest["Page Number"] * clientRequest["Entries Number"]
                higherModuleID = lowerModuleID + clientRequest["Entries Number"] - 1
                
                cursor.execute("""
                    SELECT *
                    FROM modules
                    WHERE moduleID BETWEEN ? AND ?
